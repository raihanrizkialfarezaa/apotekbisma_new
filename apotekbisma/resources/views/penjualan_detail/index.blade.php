        ...
        <div class="box">
            <div class="box-body">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <form action="{{ route('transaksi.simpan') }}" class="form-penjualan" method="post" autocomplete="off">
                            @csrf
                            <input type="hidden" name="id_penjualan" value="{{ $id_penjualan }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-8">
                                    <input type="date" id="waktu_transaksi" class="form-control waktu" name="waktu_tanggal" autocomplete="off" data-default-value="" data-preserve-server-time="{{ !empty($isEditTransaction) ? '1' : '0' }}" data-server-date="{{ !empty($isEditTransaction) && isset($penjualan->waktu) && $penjualan->waktu ? \Carbon\Carbon::parse($penjualan->waktu, config('app.timezone'))->format('Y-m-d') : '' }}" data-server-time="{{ !empty($isEditTransaction) && isset($penjualan->waktu) && $penjualan->waktu ? \Carbon\Carbon::parse($penjualan->waktu, config('app.timezone'))->format('H:i:s') : '' }}" value="">
                                </div>
                            </div>
                            <div class="form-group row">
...
<script>
    ...
    function syncDraftStockSummary(productId, summaryHtml) {
        if (!productId || !summaryHtml) {
            return;
        }

        $('.draft-stock-summary[data-product-id="' + productId + '"]').html(summaryHtml);
    }
        ...
        function updateQuantity($input, id, jumlah) {
            ...
            $.post(`{{ url('/transaksi') }}/${id}`, {
                ...
                .done(response => {
                    ...
                    if (response.data && response.data.id_produk && response.data.stock_summary_html) {
                        syncDraftStockSummary(response.data.id_produk, response.data.stock_summary_html);
                    }
    ...
    function deleteData(url) {
        if (confirm('Yakin ingin menghapus data terpilih?')) {
            $.post(url, {
                ...
                .done((response) => {
                    const updateRemainingSummary = function() {
                        if (response && response.id_produk && response.stock_summary_html) {
                            syncDraftStockSummary(response.id_produk, response.stock_summary_html);
                        }
                    };
