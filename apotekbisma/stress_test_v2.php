...
foreach ($produkWithRekaman as $produkId) {
    ...
    $produkObj = Produk::find($produkId);
    $produkName = $produkObj ? $produkObj->nama_produk : "ID: {$produkId}";
        ...
        if ($prevSisa !== null && $idx > 0) {
            ...
            $isManualAdjustment = (
                ...
                (strpos($keterangan, 'Stock Opname') !== false || 
                 strpos($keterangan, 'Penyesuaian') !== false ||
                 strpos($keterangan, 'Manual') !== false)
            );
...
if (empty($penjualanQtyMismatch)) {
    ...
    foreach (array_slice($penjualanQtyMismatch, 0, 10) as $m) {
        $produkObj = Produk::find($m['id_produk']);
        $produkName = $produkObj ? $produkObj->nama_produk : "ID:{$m['id_produk']}";
...
out("=======================================================", $output);
out("   TEST 8: TIMESTAMP CONSISTENCY", $output);
out("=======================================================", $output);
out("", $output);
